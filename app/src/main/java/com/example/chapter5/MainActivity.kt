...
class MainActivity : AppCompatActivity() {
    ...
    fun clearClicked(view: View) {
        ...
        operatorText.clear()
        updateEquationTextView()
        binding.equationTextView.text = "0"
    ...
    fun operatorClicked(view: View) {
        ...
        if(secondNumberText.isNotEmpty()) {
            Toast.makeText(this, "한개의 연산자에 대해서만 연산이 가능합니다.", Toast.LENGTH_SHORT).show()
            return
        }
    ...
    private fun updateEquationTextView() {
        val firstFormattedNumber = if(firstNumberText.isNotEmpty()) decimalFormat.format(
            firstNumberText.toString().toBigDecimal()) else ""
        val secondFormattedNumber = if(secondNumberText.isNotEmpty()) decimalFormat.format(
            secondNumberText.toString().toBigDecimal()) else ""
